...
export function CostCard({ metrics, modelName }: { metrics: MetricsState; modelName: string }) {
  ...
  return (
    <div className="bg-bg-secondary rounded border border-border border-l-[3px] border-l-accent p-3">
      <div className="text-[10px] text-text-secondary uppercase tracking-wider mb-1 font-mono">Est. Cost</div>
      <div className="text-2xl font-display font-bold text-text-primary">
        {formatCost(metrics.estimatedCost)}
      </div>
      <div className="mt-1.5">
        <span className="text-[9px] font-mono bg-bg-panel text-text-secondary rounded px-1.5 py-0.5">
          {shortModel}
        </span>
      </div>
    </div>
