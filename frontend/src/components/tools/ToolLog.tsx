...
export const ToolLog = memo(function ToolLog({ toolCalls }: { toolCalls: ToolCallState[] }) {
  ...
  return (
    <div className="bg-bg-secondary rounded border border-border border-l-[3px] border-l-accent p-3">
      ...
      {sorted.length === 0 && (
        <p className="text-[10px] text-text-secondary italic">No tool calls yet</p>
      )}
      <div role="log" aria-label="Tool call log" className="space-y-1 max-h-[200px] overflow-y-auto">
        {sorted.map((tc) => (
          <div key={tc.id} className="flex items-center gap-1.5 text-[10px]">
            <Wrench size={10} className="text-text-secondary shrink-0" aria-hidden="true" />
            <span className="font-mono text-text-primary truncate">{tc.name}</span>
            <span className="text-text-secondary truncate">{tc.agent.split(" ").pop()}</span>
            {tc.latency_ms > 0 && (
              <span className="font-mono text-text-secondary shrink-0">{Math.round(tc.latency_ms)}ms</span>
            )}
            <span className="shrink-0" role="status" aria-label={tc.status === "success" ? "Success" : tc.status === "error" ? "Error" : "Pending"}>
              {tc.status === "success" ? (
                <Check size={10} className="text-success" aria-hidden="true" />
              ) : tc.status === "error" ? (
                <X size={10} className="text-error" aria-hidden="true" />
              ) : (
                <span className="w-2.5 h-2.5 rounded-full bg-warning animate-pulse-fast inline-block" aria-hidden="true" />
              )}
            </span>
          </div>
        ))}
