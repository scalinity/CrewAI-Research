  ...
  return (
    ...
    >
      <div className="shrink-0" style={{ WebkitAppRegion: "no-drag" } as React.CSSProperties}>
        <span className="font-mono font-semibold text-[9px] uppercase tracking-[0.2em] text-accent-blue/70 block leading-none mb-0.5">
          Mission Control
        </span>
      ...
      <div className="flex items-center gap-2 shrink-0">
        ...
        <span
          className={`inline-block w-1.5 h-1.5 rounded-full ${connectionStatus === "connected" ? "bg-accent-emerald" : "bg-accent-rose"}`}
          role="status"
      ...
      {isRunning && (
        <motion.div
          className="absolute bottom-0 left-0 right-0 h-px bg-accent-blue/40"
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          style={{ boxShadow: "0 0 8px var(--color-glow-blue)" }}
        />
      )}
    </motion.header>
  );
